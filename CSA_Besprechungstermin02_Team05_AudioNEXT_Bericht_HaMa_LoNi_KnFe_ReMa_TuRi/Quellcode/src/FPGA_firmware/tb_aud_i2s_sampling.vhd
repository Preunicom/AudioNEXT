...
entity tb_aud_i2s_sampling is
end tb_aud_i2s_sampling;

architecture Testbench of tb_aud_i2s_sampling is
    ...
    begin
        ...
        for i in 23 downto 0 loop
            s_data <= c_data_l(i);
            wait until rising_edge(s_mclk); -- meta-FF
            wait until rising_edge(s_mclk); -- sync-FF
            s_sclk <= '1';
            wait until rising_edge(s_mclk); -- DUT sampelt
            s_sclk <= '0';
            wait until rising_edge(s_mclk);
        end loop;
...
begin
    ...
    begin
        ...
        send_frame(X"000000", X"FFFFFF", mclk, sclk, lrck, data_in);
        -- frame 2: nur MSB / nur LSB
        send_frame(X"800000", X"000001", mclk, sclk, lrck, data_in);
